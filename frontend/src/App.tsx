...
/**
 * 404 scenario not found => we show <ScenarioNotFound>.
 * About => we show <AboutPage>.
 * Otherwise => normal app flow.
 * But we do that logic inside the component.
 */
...
const App: React.FC = () => {
  ...
  /**
   * Update the URL when the scenario changes.
   * In production (non-dev mode), we always use subdomains rather than query parameters.
   */
  function handleScenarioChange(newScenario: string): void {
    ...
    if (!isDevMode) {
      ...
      if (parts.length === 2) {
        // Append the new scenario as a subdomain.
        url.hostname = `${newScenario}.${url.hostname}`;
      } else {
        // Replace the existing subdomain.
        parts[0] = newScenario;
...
<svg width="80" height="60" viewBox="0 0 442.48889 415.55103" xmlns="http://www.w3.org/2000/svg">
  <path fill="lightblue" stroke="#333" stroke-width="2"
     d="m 402.68219,327.7504 -19.25,-9.7 ... z"
...
<svg width="80" height="60" viewBox="0 0 442.48889 415.55103" xmlns="http://www.w3.org/2000/svg">
  <path fill="lightblue" stroke="#333" stroke-width="2"
     d="m 398.35219,330.7904 ... z"
     title="Victoria"
     id="AU-VIC" />
</svg>
    `
                };
                return svgs[regionKey] || '<svg width="80" height="60"></svg>';
              })() }}
            />
            {usedIntervalDate && (
